...
class FormatAnnotationIntrospector extends NopAnnotationIntrospector {
  ...
  private TypeDescriptor getTypeDescriptorForDeserializer(AnnotatedMethod method) {
    // TODO - Could handle annotated getter/is/setter method here - those should probably take
    // precedence over field annotations
    Assert.isTrue(method.getName().startsWith("set"),
        "Expected a setter method, but was " + method.getName());
  ...
  private TypeDescriptor getTypeDescriptorForSerializer(AnnotatedMethod method) {
    // TODO - Could handle annotated getter/is methods here - those should probably take precedence
